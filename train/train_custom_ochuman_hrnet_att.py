...
###
##attention module
from attention.MODELS.cbam import *
from attention.MODELS.bam import *
###
...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels, num_channels, fuse_method, multi_scale_output=True, att_type=None):
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1, att_type=None):
        downsample = None
        if stride != 1 or \
...
class PoseHighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, att_type=None, **kwargs):
        self.inplanes = 64
        ...
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(Bottleneck, 64, 4, att_type=att_type)
...
def get_pose_net(is_train, **kwargs):   
    ...
    model = PoseHighResolutionNet(cfg, att_type, **kwargs)
...
def main():   
    ...
    for epoch in range(0, END_EPOCH):
        ...
        #perf_indicator = 0.0
        perf_indicator = validate(cfg, valid_loader, valid_dataset, model, criterion, final_output_dir, epoch)
    ...
    validate(cfg, test_loader, test_dataset, model, criterion, final_output_dir, epoch)
